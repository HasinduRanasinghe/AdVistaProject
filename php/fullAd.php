...
<body>
	...
	<div class="header">
		...
		<!-- <span><button class="postButton">Post Ad</button></span> -->
		<span>
	...
	<div class="bodyDiv">
		...
		<?php
			// capturing the ID of the advertisement which customer is clicked on
			$adID = intval($_GET["adID"]);

			// starting the database connection
			global $con;
			$con = new mysqli("localhost", "root", "", "advistalk");
			$sqlCmd = "SELECT * FROM advertisement WHERE adID = $adID";
			$resultSet = $con->query($sqlCmd);
			$row = $resultSet->fetch_assoc();
			$con->close();

			//rendering the details of the  captured adverisement

			echo '<h2 class="fullAdDetails">'.$row["title"].'</h2>';
			echo '<center><hr class="adDetails-hr"></center>';
			echo '<h3 class="fullAdDetails">'.$row["category"].'</h3>';
			echo '<center><hr class="adDetails-hr"></center>';
			echo '<p class="fullAdDetails">'.$row["subCategory"].' - '.$row["type"].'</p>';
			echo '<center><hr class="adDetails-hr"></center>';

			echo '<center>
			<div class="imageLine">
					<div class="imagesInLine1" style="background-image: url(../images/advertisements/'.$row["adID"].'a.jpg);"></div>
					<div class="imagesInLine1" style="background-image: url(../images/advertisements/'.$row["adID"].'b.jpg);"></div>
					<div class="imagesInLine1" style="background-image: url(../images/advertisements/'.$row["adID"].'c.jpg);"></div>

					<div class="imagesInLine1" style="background-image: url(../images/advertisements/'.$row["adID"].'d.jpg);"></div>
					<div class="imagesInLine1" style="background-image: url(../images/advertisements/'.$row["adID"].'e.jpg);"></div>
			</div>
			</center>';

			echo '<div class="adDescription">'.$row["description"].'</div>';
			echo '<center><hr class="adDetails-hr"></center>';

			echo '<h3 class="fullAdDetails">'.$row["price"].'/=</h3>';
			echo '<center><hr class="adDetails-hr"></center>';
			echo '<p class="fullAdDetails">Published by   : '.$row["name"].' - '.$row["email"].'</p>';
			echo '<p class="fullAdDetails">Contact number : '.$row["phone1"].'</p>';
		?>
	</div>
